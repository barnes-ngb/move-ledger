...
**The box screens undershoot doc 04 in three places.** Found while running APPLY-05 and left alone, because the plan supplied the markup and changing it is a design decision rather than a fix. Doc 04 section 2 sets a 72px minimum for the number on Add box; the plan wrote `text-6xl`, which is 60px. Doc 04 section 3 says the color swatch is a background fill rather than a small dot; Add box prints the color name in colored text and the room chips use 24px dots. Doc 04 section 4 asks Find for the last five recent boxes and a link to text search; Find shows matches only, and text search does not exist until step 10. Decide each of these on a phone, then either change the code or amend doc 04.
